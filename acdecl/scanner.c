...
enum TokenTag classify_string() {
  ...
  case 'c':
    if (scanner.current - scanner.start > 1) {
      ...
      case 'h':
        return checkKeyword(2, 2, "ar", TOKEN_TYPE);
      case 'o':
        switch (scanner.start[2]) {
        case 'm':
          return checkKeyword(3, 4, "plex", TOKEN_TYPE);
        case 'n':
          return checkKeyword(2, 3, "nst", TOKEN_QUALIFIER);
        }
      }
    }
    break;
  case 'e':
    return checkKeyword(1, 3, "num", TOKEN_TYPE);
  case 'f':
    return checkKeyword(1, 4, "loat", TOKEN_TYPE);
  case 'i':
    return checkKeyword(1, 2, "nt", TOKEN_TYPE);
  case 's':
    if (scanner.current - scanner.start > 1) {
      switch (scanner.start[1]) {
      case 'h':
        return checkKeyword(2, 2, "ar", TOKEN_TYPE);
      case 'i':
        return checkKeyword(2, 4, "gned", TOKEN_TYPE);
      case 't':
        return checkKeyword(2, 4, "ruct", TOKEN_TYPE);
      }
    }
    break;
  case 'u':
    if (scanner.current - scanner.start > 1) {
      ...
      case 'n':
        switch (scanner.start[2]) {
        case 'i':
          return checkKeyword(3, 2, "on", TOKEN_TYPE);
        case 's':
          return checkKeyword(3, 5, "igned", TOKEN_TYPE);
        }
      }
    }
    break;
  case 'v':
    return checkKeyword(2, 7, "olatile", TOKEN_QUALIFIER);
  }
